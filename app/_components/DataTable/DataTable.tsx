...
const iconRowSx   = { display: "flex", flexShrink: 0, alignItems: "center", gap: 0.5 };
const viewBtnSx   = { fontSize: "0.65rem", py: 0.2, px: 1, minWidth: 0 };
const editIconSx  = { opacity: 0.5, "&:hover": { opacity: 1 } };
const popoverBoxSx = { p: 2, width: 280, display: "flex", flexDirection: "column", gap: 1.5 };
...
export default function DataTable({ sessions, type, onAssignGrade, setSessions }: Props) {
    ...
    <>
                            ...
                            >
                              <Box sx={popoverBoxSx}>
                                <TextField
                                  ...
                                  onChange={(e) =>
                                    setGradeValues((prev) => ({ ...prev, [rowKey]: e.target.value }))
                                  }
                                />
                                <Box sx={actionRowSx}>
                                  <Button variant="outlined" color="inherit" size="small"
                                    disabled={loadingSessionId === rowKey} onClick={handleClose}>
                                    Cancel
                                  </Button>
                                  <Button
                                    variant="contained"
                                    color="primary"
                                    size="small"
                                    disabled={loadingSessionId === rowKey || isGradeInvalid(rowKey)}
                                    onClick={() => handleGradeSubmit(session?.sessionId, rowKey)}
                                    startIcon={loadingSessionId === rowKey ? <CircularProgress size={14} color="inherit" /> : null}
                                  >
                                    {loadingSessionId === rowKey ? "Saving..." : "Assign grade"}
                                  </Button>
                                </Box>
                              </Box>
                            </Popover>
      ...
      <Dialog open={notesDialog} onClose={() => setNotesDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          {notesFetching ? (
            <Box sx={centerSx}><CircularProgress size={24} /></Box>
          ) : (
            <TextField fullWidth multiline rows={5} size="small" label="Notes"
              value={editedNotes}
              onChange={(e) => { setEditedNotes(e.target.value); setNotesSuccess(false); }}
            />
          )}
          {notesError   && <Typography variant="caption" color="error"        sx={{ mt: 1, display: "block" }}>{notesError}</Typography>}
          {notesSuccess && <Typography variant="caption" color="success.main" sx={{ mt: 1, display: "block" }}>Notes saved successfully.</Typography>}
        </DialogContent>
        <DialogActions>
          <Button variant="outlined" color="inherit" onClick={() => setNotesDialog(false)}>Cancel</Button>
          <Button
            variant="contained"
            color="primary"
      ...
      <Dialog open={!!viewNotesSession} onClose={() => setViewNotesSession(null)} maxWidth="sm" fullWidth>
        <DialogTitle>
          ...
          {viewNotesSession && (
            <Typography variant="body2" color="text.secondary">
              {viewNotesSession.studentFirstName} {viewNotesSession.studentLastName} — {viewNotesSession.subject}
            </Typography>
          )}
        </DialogTitle>
        <DialogContent dividers>
